function zero = newton_cena_old(f, x0, maxit, ftol, q)
f_x = expand(f);
f_dx = diff(f_x);
coef_x = sym2poly(f_x);
...
for k=1:maxit
    ...
    for i=1:length(coef_x)
        ...
        pow_val = pow_val + pow_err;
        [mul_val, mul_err] = TwoProduct(coef_x(i), pow_val, q);
        f_x_vals(i) = mul_val + mul_err;
    ...
    sprintf('%.7e | %.7e | %.7e | %.7e | %.7e',k,x0,f_x_true,f_dx_true)
    
    
    x0 = t-e;
    if abs(f_x_true) < ftol
        disp('in tol')
        break
    end
